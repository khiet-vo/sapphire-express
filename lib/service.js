...
module.exports.Service = new Class({
	...
	csrfCheck : function(req, res)
	{
		...
		console.log('DEBUG ON - SERVER \n', cookieCsrf,'\n', submitCsrf);
		if (!cookieCsrf) return {success: false, error: 'REQUEST_DENIED'}

		if (!submitCsrf) return {success: false, error: 'REQUEST_DENIED'}

		if (cookieCsrf != submitCsrf) return {success: false, error: 'REQUEST_DENIED'}
